...
export const CONFIG_CATALOG: ConfigCatalogSpec[] = [
  ...
  {
    key: 'INGEST_ARTICLE_NORMALIZATION',
    category: 'pipeline',
    // Read per-call in EntityUpsertService (process.env), never
    // constructor-captured — a flip takes effect without restart.
    defaultValue: '0',
    ...
    isBooleanFlag: true,
    description:
      'Article-insensitive entity reuse (the state-transitions s07 entity-split class): the extractor coins one referent with and without a leading English article across turns ("the office lease" vs "office lease"), splitting it into two entities — every slot-keyed consumer (conflict formation, timelines, competing pairs) then goes blind to the collision. On, the canonical-name lookup is widened to the leading-article VARIANTS of the name (stripped + the/a/an re-prefixed) after the exact match misses; only a UNIQUE variant match is reused, stored canonicalName is never rewritten, and the same tenant-global userId IS NONE fence applies. Off (default) → byte-identical, per-article twins form as before.',
  },
  ...
  },
  {
    ...
    isBooleanFlag: true,
    description:
      "Succession tiebreaker for the bitemporal close-margin doctrine (migration 0129). The CONFLICT_MENTION_FACT_SLOT promotion plus the slot-exact cosine floor route mention-path single-value slots into the margin doctrine, where batch-shaped corpora score every write identically (margin ~0) and every same-slot pairing lands COMPETING — genuine temporal updates included, so honest serving abstains on settled current values (memory-fitness regression, run mfmtqn2jiq). Score margin cannot tell an UPDATE from a CONTRADICTION, and measured live neither can validFrom separation (contradiction arms sit 6-15 days apart, the killing update pairing 25 minutes) or origin identity (one recorder per corpus). When on, a mention-path close-margin 'bitemporal' write whose object carries a deterministic succession cue ('is now X', 'moves from A to B', 'instead of', 'no longer', 'switched to' — word-bounded regex, no LLM) closes pool members whose validFrom is strictly earlier by more than CONFLICT_TEMPORAL_TIEBREAK_WINDOW_MS as superseded history (the natural-supersede sentinel; audit trail in conflictTrace.temporalTiebreak) instead of flipping them to COMPETING; everyone else — cue-less contradiction arms ('actually ends in September 2026'), same-stamp pairs, within-window members, missing validFrom — keeps the close-margin → COMPETING doctrine, 0085-partition style. The direct typed path never computes the cue, so its shapes (the D6 payout-cutoff competing pair included) are untouched. Off (default) = the fn's new option args are never bound, byte-identical.",
  },
  {
  ...
  },
  {
    ...
    isBooleanFlag: true,
    description:
      'Language-agnostic state-transition classifier lane (semantic stage of the state-transition battery follow-up, generalizing EXTRACTOR_STATE_VERB_HARVEST to out-of-lexicon verbs and Russian): compromise-based English morphology plus a bounded Russian past-tense matcher find candidate verb clauses (past-tense, non-negated, non-hypothetical, with a complement), then embedding-prototype matching classifies each clause as completed_acquire / completed_dispose / completed_change / intention / unrelated by max-cosine against a fixed EN+RU prototype bank (BGE-M3 is multilingual, so the classifier generalizes across input languages by construction). Accepted clauses (completed_* above the calibrated score floor + runner-up margin — see eval:transition-calibration) land as span-grounded `state_change` facts with the same holder binding as the lexicon lane; a sentence the lexicon lane already harvested is deferred whole, so the two lanes compose without double emission. Designed and calibrated for EMBEDDER_PROVIDER=bge-m3; with the OpenAI embedder the thresholds are uncalibrated. Off (default) → byte-identical extraction, the embedder is never called. Requires re-ingest.',
  },
  {
