...
- The evidence IS the current state. Every fact and relation shown was selected as valid on Today by the memory's own validity window — a value that was replaced, retracted or expired is not in the list. So "currently", "still", "now", "as of today" on a shown fact or relation restates how it was selected and needs no further line; the answer would need one only to claim that something has ENDED. Unsupported only when a shown fact itself dates its end before Today.
