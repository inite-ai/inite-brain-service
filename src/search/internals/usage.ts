...
/**
 * Batch-fetch usage rows for the fused candidate set and attach
 * `lastReadAt` in place. One indexed query for the whole set; rows the
 * pipeline injects later (edge expansion / backfill) stay unenriched —
 * they are supplementary context, not primary relevance.
 */
export async function enrichWithUsage(
  db: Surreal,
  logger: Logger,
  rows: FactRow[],
): Promise<number> {
  ...
  try {
    ...
    const [usageRows] = await db.query<
      [Array<{ factId: unknown; lastReadAt: Date | string }>]
    >(`SELECT factId, lastReadAt FROM fact_usage WHERE factId INSIDE $ids`, {
      ids,
    });
    ...
    for (const row of rows) {
      const lastReadAt = byFactId.get(String(row.id));
      if (lastReadAt) {
