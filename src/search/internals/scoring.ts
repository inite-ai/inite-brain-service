...
import { activationDecay } from './activation';
...
/**
 * The activation input of one row (activation.ts): its age, and the
 * verified uses attached to it (0107 — the count under
 * RETRIEVAL_VERIFIED_USE_RANKING, the last use's time under
 * RETRIEVAL_VERIFIED_USE_DECAY) or the legacy last-retrieval stamp
 * (SEARCH_USAGE_DECAY_ENABLED). An unenriched row fades from recordedAt
 * alone. Kept out of the scoreRows map arrow for the seam complexity
 * budget.
 */
function activationOf(
  ...
  const daysSince = (iso: string | undefined): number | undefined =>
    iso ? (now - new Date(iso).getTime()) / 86_400_000 : undefined;
  return activationDecay({
    ageDays: daysSince(row.recordedAt) ?? 0,
    halfLifeDays,
    uses: row.verifiedUseScore,
    lastUseDays: daysSince(row.lastVerifiedUseAt),
    lastReadDays: daysSince(row.lastReadAt),
  });
}
...
export function scoreRows({
  ...
  return rows.map((row) => {
    ...
    // How available the fact is from its history of use (activation.ts):
    // written, then every verified use, each a power-law trace. No
    // half-life (a state, an instruction) = it does not fade.
    const decay =
      policy.decayHalfLifeDays === null ? 1 : activationOf(row, now, policy.decayHalfLifeDays);
