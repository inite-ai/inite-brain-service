...
export class IngestMentionDto {
  ...
  /**
   * IANA timezone of the speaker's session (e.g. 'Asia/Tokyo'). When
   * event-time extraction resolves a relative expression ("yesterday", "3
   * days ago"), this anchors it to the speaker's LOCAL calendar day so a
   * message near a UTC day boundary isn't shifted. Absent or unknown ⇒
   * UTC-day behavior — never rejects the ingest.
   */
  @IsOptional()
  @IsString()
  @MaxLength(64)
