...
import { docxText, pptxText, xlsxText, decodeEntities } from '../src/evidence/processing/adapters/ooxml-text';
...
function input(bytes: Buffer, mediaType: string): ProcessorInput {
  return {
    asset: { id: 'a', modality: 'document', mediaType, availability: 'hot', byteLength: bytes.length },
...
describe('zip-reader — bounded, named-parts-only', () => {
  ...
  it('refuses a declared bomb, a real bomb, an encrypted part and a directory over the cap', () => {
    ...
    expect(() => readZipParts(declared, { select: () => true, maxPartBytes: 1000 })).toThrow(/declares 10000 bytes/);
    ...
    expect(() => readZipParts(real, { select: () => true, maxPartBytes: 1000 })).toThrow(/failed to inflate/);
    const enc = buildZip([{ name: 'z', data: 'secret', encrypted: true }]);
    expect(() => readZipParts(enc, { select: () => true, maxPartBytes: 1000 })).toThrow(/encrypted/);
    const many = buildZip(Array.from({ length: 5 }, (_, i) => ({ name: `p${i}`, data: 'x' })));
    expect(() => readZipParts(many, { select: () => true, maxPartBytes: 1000, maxEntries: 4 })).toThrow(/cap 4/);
    expect(() => readZipParts(Buffer.from('not a zip at all'), { select: () => true, maxPartBytes: 10 })).toThrow(/not a ZIP/);
  });
});

describe('ooxml-text', () => {
  it('docx: paragraphs become lines, table cells tabs, entities decode, media is never read', () => {
    const text = docxText(docxBytes(['Acme Robotics <2019>', 'CTO: Maria &amp; co', ['name', 'role'], 'Orbit arm']), LIMITS);
  ...
  it('xlsx: sheets in workbook order with markers, shared strings and numbers, tab-separated', () => {
    const text = xlsxText(
      xlsxBytes([
        { name: 'Vendors', rows: [['vendor', 'part'], ['Nidec', 'motor'], ['Maxon', 42]] },
        { name: 'Notes & more', rows: [['backup ok']] },
      ]),
      LIMITS,
    );
    expect(text).toBe('[sheet: Vendors]\nvendor\tpart\nNidec\tmotor\nMaxon\t42\n[sheet: Notes & more]\nbackup ok');
  });

  it('xlsx: without a workbook part the sheets fall back to part-number order', () => {
    const zip = buildZip([
      { name: 'xl/worksheets/sheet10.xml', data: '<worksheet><sheetData><row><c t="inlineStr"><is><t>ten</t></is></c></row></sheetData></worksheet>' },
      { name: 'xl/worksheets/sheet2.xml', data: '<worksheet><sheetData><row><c><v>2</v></c></row></sheetData></worksheet>' },
    ]);
  ...
  it('names a container that carries no text part', () => {
    expect(() => docxText(buildZip([{ name: 'other.xml', data: '<x/>' }]), LIMITS)).toThrow(/no word\/document.xml/);
    expect(() => pptxText(buildZip([{ name: 'ppt/presentation.xml', data: '<x/>' }]), LIMITS)).toThrow(/no slide parts/);
  });

  it('decodeEntities handles named, decimal and hex forms and leaves the unknown alone', () => {
    expect(decodeEntities('a&amp;b &lt;c&gt; &#65;&#x42; &bogus; &#xFFFFFFFF;')).toBe('a&b <c> AB &bogus; &#xFFFFFFFF;');
  });
});
...
describe('OfficeTextAdapter', () => {
  ...
  it('accepts the three macro-free OOXML types on the document modality only', () => {
    expect(adapter.accepts('document', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document; x=1')).toBe(true);
    expect(adapter.accepts('document', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')).toBe(true);
    expect(adapter.accepts('document', 'application/vnd.openxmlformats-officedocument.presentationml.presentation')).toBe(true);
    expect(adapter.accepts('document', 'application/vnd.ms-word.document.macroenabled.12')).toBe(false);
    expect(adapter.accepts('document', 'application/pdf')).toBe(false);
    expect(adapter.accepts('image', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')).toBe(false);
  ...
  it('produces one asset-level text representation per document', async () => {
    const out = await adapter.process(
      input(docxBytes(['Hello', 'World']), 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'),
    );
  ...
  it('fails a text-free document by name rather than writing an empty representation', async () => {
    await expect(
      adapter.process(input(docxBytes([]), 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')),
...
describe('mail-text', () => {
  ...
  it('bounds the part walk', () => {
    const parts = Array.from({ length: 30 }, (_, i) => `--b\r\nContent-Type: text/plain\r\n\r\npart ${i}\r\n`).join('');
