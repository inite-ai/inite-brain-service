...
const PACK = {
  ...
  sources: [
    { id: 'notes', kind: 'native', connector: 'fs', shape: 'document', defaults: { contentPolicy: 'text' } },
    { id: 'listing', kind: 'native', connector: 'fs', shape: 'document', defaults: { contentPolicy: 'manifest' } },
  ],
};
...
describe('agent sync protocol (e2e)', () => {
  ...
  beforeAll(async () => {
    for (const k of ['SOURCE_PLANE_ENABLED', 'DOCUMENT_INGEST_ENABLED', 'WORKER_LOOP_ENABLED', 'SOURCE_KIND_FS']) {
    ...
    const installed = await f.http.post('/v1/admin/packs').set(auth()).send({ manifest: PACK, acceptSources: true });
    expect([200, 201]).toContain(installed.status);
    for (const [sourceId, label] of [['notes', 'Laptop notes'], ['listing', 'Listing only']] as const) {
      const r = await f.http
        .post('/v1/admin/source-connections')
        .set(auth())
        .send({ packId: 'notes_pack', sourceId, vertical: 'notes', label, host: HOST, config: { root: '/Users/me/notes' } });
  ...
  const item = (externalId: string, revision: string) => ({
    type: 'upsert',
    item: { externalId, path: externalId, title: externalId, mediaType: 'text/markdown', revision, originUri: `file:///Users/me/notes/${externalId}` },
  });
  ...
  it('lists the agent host’s connections with their pack entries; a server host sees none', async () => {
    ...
    const notes = mine.body.connections.find((c: { connection: { id: string } }) => c.connection.id === connectionId);
    ...
    const other = await f.http.get('/v1/source-connections').query({ host: 'agent:other' }).set(auth());
  ...
  it('first run: begin is full, deltas name what to fetch, items land through the door, finish records the checkpoint', async () => {
    ...
    expect(begun.body).toMatchObject({ full: true, checkpoint: null, contentPolicy: 'text', fetchBudget: null });
    firstRun = begun.body.runId;

    const deltas = await post(`/agent-runs/${firstRun}/deltas`, {
      deltas: [item('readme.md', 'r1'), item('guide/onboarding.md', 'r1'), item('vendors.md', 'r1'), { type: 'checkpoint', checkpoint: { files: 3 } }],
    });
    ...
    expect([...deltas.body.fetch].sort()).toEqual(['guide/onboarding.md', 'readme.md', 'vendors.md']);
    ...
    for (const [id, text] of [
      ...
      const r = await post(`/agent-runs/${firstRun}/items`, { externalId: id, item: doc(text, id) });
      expect(r.status).toBe(201);
    ...
    expect(finished.body).toMatchObject({ mode: 'full', status: 'succeeded', seen: 3, new: 3, fetched: 3, ingested: 3, gone: 0, closed: 0 });
    ...
    expect(job.body).toMatchObject({ jobType: 'source_sync', status: 'succeeded', triggeredByActor: HOST });
  ...
  it('incremental run: only a moved revision is fetched; an explicit gone closes; the fences hold', async () => {
    ...
    const deltas = await post(`/agent-runs/${runId}/deltas`, {
      deltas: [item('readme.md', 'r2'), item('guide/onboarding.md', 'r1'), { type: 'gone', externalId: 'vendors.md' }],
    });
    expect(deltas.body).toMatchObject({ seen: 2, new: 0, changed: 1, unchanged: 1, gone: 1, fetch: ['readme.md'] });
    const uncatalogued = await post(`/agent-runs/${runId}/items`, { externalId: 'nope.md', item: doc('x', 'x') });
    expect(uncatalogued.status).toBe(404);
    const changed = await post(`/agent-runs/${runId}/items`, {
      externalId: 'readme.md',
      item: doc('Acme Robotics was founded in 2019 in Tallinn. The CTO is Maria Lind.', 'readme.md'),
    });
    expect(changed.body).toEqual({ status: 'ingested' });

    const finished = await post(`/agent-runs/${runId}/finish`, { status: 'succeeded', checkpoint: { files: 2 } });
    expect(finished.body).toMatchObject({ mode: 'incremental', seen: 2, changed: 1, unchanged: 1, gone: 1, fetched: 1, ingested: 1 });
    expect(finished.body.closed).toBeGreaterThanOrEqual(1);
    // A finished run takes no more calls.
    expect((await post(`/agent-runs/${runId}/deltas`, { deltas: [item('x', '1')] })).status).toBe(409);
  ...
  it('a full run marks what it did not see gone; a failed run records the error; manifest policy skips content', async () => {
    ...
    const gone = await f.http.get(`/v1/admin/source-connections/${connectionId}/items?state=gone`).set(auth());
    expect(gone.body.items.map((i: { externalId: string }) => i.externalId).sort()).toEqual(['guide/onboarding.md', 'vendors.md']);

    const failing = await post('/agent-runs', { agentId: AGENT });
    const failed = await post(`/agent-runs/${failing.body.runId}/finish`, { status: 'failed', error: 'disk unplugged' });
    ...
    const listing = await f.http.post(`/v1/source-connections/${listingId}/agent-runs`).set(auth()).send({ agentId: AGENT });
    expect(listing.body.contentPolicy).toBe('manifest');
    const lr = listing.body.runId as string;
    const ld = await f.http.post(`/v1/source-connections/${listingId}/agent-runs/${lr}/deltas`).set(auth()).send({ deltas: [item('a.md', '1')] });
    expect(ld.body.fetch).toEqual(['a.md']);
    const li = await f.http.post(`/v1/source-connections/${listingId}/agent-runs/${lr}/items`).set(auth()).send({ externalId: 'a.md', item: doc('x', 'a') });
    expect(li.body).toMatchObject({ status: 'skipped' });
    const lf = await f.http.post(`/v1/source-connections/${listingId}/agent-runs/${lr}/finish`).set(auth()).send({ status: 'succeeded' });
