...
describe('PolicyResolverService', () => {
  ...
  it('resolves claim names to compiled sets', async () => {
    const { resolver } = makeResolver();
    const ctx = await resolver.contextFor('co_a', {
      keyHash: 'sha256:k',
      claimNames: ['readonly-agent'],
    });
  ...
  it('skips DISABLED sets silently (known name ≠ dangling reference)', async () => {
    const { resolver, resolutionErrors } = makeResolver({
      policyRows: [{ name: DISABLED_DOC.name, mode: 'disabled', document: DISABLED_DOC }],
    });
    const ctx = await resolver.contextFor('co_a', {
      keyHash: 'sha256:k',
      claimNames: ['switched-off'],
    });
    expect(ctx).toBeNull();
