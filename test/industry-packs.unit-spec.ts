...
const INDUSTRY: DomainPackManifest[] = FIRST_PARTY_PACKS.filter((p) => p.id !== 'real_estate');

describe.each(INDUSTRY.map((p) => [p.id, p] as const))('industry pack: %s', (id, pack) => {
  ...
  it('has a committed JSON artifact matching the TS source', () => {
    const file = join(__dirname, '..', 'packs', `${id.replace(/_/g, '-')}.pack.json`);
...
describe('first-party pack library', () => {
  it('lists the industry packs + real_estate', () => {
    ...
    expect(ids).toEqual(['file_memory', 'fintech', 'hr', 'insurance', 'legal', 'medical', 'real_estate', 'web_memory']);
  });
