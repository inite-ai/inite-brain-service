...
describe('scoreRows — G8 usage ranking factor', () => {
  ...
  it('a verified-use count does not touch the G8 usage factor', () => {
    const rows = [row({ readCount: 10 }), row({ readCount: 10, verifiedUseScore: 5 })];
    const [plain, verified] = scoreRows({ rows, now: NOW, usageBeta: 0.5 });
    expect(verified!.breakdown.usage).toEqual(plain!.breakdown.usage);
  });
});
