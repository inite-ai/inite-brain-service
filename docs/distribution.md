...
| npm `@inite/brain-agent` | ⬜ **not published** — the local agent (`install` / `status` / `doctor`, launchd + systemd); the admin UI prints `npx @inite/brain-agent install …`, so this is the one the Connections page depends on. Publish via `.github/workflows/publish-clients.yml` (workflow_dispatch → `brain-agent`; needs npm trusted publishing for the package or the `NPM_TOKEN` secret). |  |
...
for LLM agents. MCP server (Streamable HTTP) exposing 28 tools across
...
```
- [inite-brain-service](https://github.com/inite-ai/inite-brain-service) — open-source memory layer for Claude (and any other MCP client). Bitemporal knowledge graph, 28 tools, three memory tiers (facts/episodes/procedural), conflict resolution, GDPR forget. AGPL-3.0.
```
