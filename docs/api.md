...
| Endpoint | Notes |
|---|---|
| `POST /v1/ingest/mention` | The simplest write: free text → entities + facts + edges. `text` is the only required field — `contextRef` defaults to the `chat` vertical, `emittedAt` to request time. `userId` scopes the write (facts AND edges) to that end user AND makes them the speaker unless a `knownEntities` anchor with `role: 'speaker'` says who else is talking; first-person turns land on the user's own entity (`{vertical: 'user', id: <userId>}`, personal), whose name the memory learns from a `name` fact — the user's own words, or `/v1/ingest/fact` on that reference at onboarding (a speaker anchor with a `name` names it too). With `INGEST_MENTION_VIA_DOCUMENT=1`, routed through the document pipeline (same response shape; `userId` scope preserved end-to-end — 0127). |
| `POST /v1/ingest/fact` | Declared structured fact ingest — the precision route when the claim is already known. Optional `userId` stamps a per-user memory scope (0055) on the FACT: scope-local conflict resolution, invisible outside that user. Identity is tenant-wide — a `{vertical, id}` reference names the same node for every user (the node the mention path resolves that name to), so a user's private claim and the shared record meet on one entity; only `{vertical: 'user', id: <userId>}` is a private node. Legacy per-user copies are folded back once per tenant (`POST /v1/admin/maintenance/entities/consolidate-scoped` to run it on demand). |
...
| Endpoint | Notes |
|---|---|
| `POST /v1/search` | Hybrid (vector + BM25), router-boosted, listwise rerank w/ self-consistency, per-leg CI, entity-fact backfill. `userId` scopes results to tenant-global + that user's personal memory (fail-closed: omitted → global only); also on `/synthesize` and `/search/multi-hop`. On `/synthesize` the userId also names the asker: the query's "I"/"my" is the user's own entity, and the answer addresses them. When the query cannot be embedded (embedder warming up or down) a hybrid search answers lexical-only and carries `degraded: ["vector_leg"]`; retry later for the full ranking. See [Architecture § Retrieval pipeline](architecture.md#retrieval-pipeline). |
...
| `GET /v1/entities/:id/timeline` | Bitemporal sweep — `fact.recorded` / `fact.retracted` events on the transaction-time axis (`at`); each recorded event also carries `validFrom`, the valid-time instant, so a reader can order by the world's time instead of the ledger's (a value ingested late still precedes what replaced it). `?since=`/`?until=` page the window; `?recordedAt=` cuts to events known by T. |
...
| `GET /v1/users/:userId/profile` | Deterministic, prompt-ready profile of one user's OWN memory (strict `userId` scope; `identity` — their own entity and the name the memory has learned, null until it has — then persona-first sections + `profileText`, whose first line is the identity). A user-bound token only fetches its own profile; M2M any. Flag `USER_PROFILE_API_ENABLED`. |
| `GET /v1/entities/:id/connections` | Typed edges + direct neighbours. Edges carry the per-user scope of the turn that wrote them (0153; the mention and document paths stamp it with the facts): omit `userId` for tenant-global only, pass it for that user's own edges and neighbours as well (fail-closed, like `/search`). |
