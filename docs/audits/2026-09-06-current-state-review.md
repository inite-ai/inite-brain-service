...
> **Статус на 2026-09-10.** Закрыто в main: F1 → #467 (`SliceTable` больше не экспортируется из route-модуля; сборка landing зелёная), F2 → #527, F4 → #532, F5 → #528, F8/F9 → #533, F3/F7 (answer-cache dependencies + порядок usage-телеметрии) → #536, F6/F10 + атомарность belief revision (доп. замечание 1) → #537. Открытых PR из этого отчёта не осталось. Класс FLEXIBLE-дрейфа (не в этом отчёте, найден позже): #513 (`job_run`) + #526 (все 43 объявления; механизм — экспортер SurrealDB 2.6.5 `--v3` теряет `FLEXIBLE` у `option<object>`). F11 — контракт `asOf` зафиксирован в `docs/bitemporal-semantics.md`; код приведён к этому контракту позже (`activeFactWhere` больше не граничит `recordedAt` в ветке только-`asOf`). Файлы `audit-review-repros.*.txt` и `brain-audit-*.log.txt` рядом — воспроизведения и логи, на которые ссылается текст.
