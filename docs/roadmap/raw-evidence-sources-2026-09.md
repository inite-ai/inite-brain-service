...
| **W6 Progressive indexing + parsers** | `SOURCE_PROGRESSIVE`: a `manifest` connection reads nothing until a query matches a catalogue row's title or path — the hit is counted, the best few (`SOURCE_DEEPEN_PER_QUERY`) are queued as a `source_sync` job with a `deepen` payload that fetches THOSE ROWS ALONE (no walk, no checkpoint, no gone policy), and the next query sees the content. ⚡The probe never changes the answer it rides on and never throws; ⚡a hit is not a read; ⚡deliberately NOT a numbered conveyor stage, because a stage has to leave something behind. Plus `EVIDENCE_PARSER_REMOTE`: the first platform processor allowed a network call — a document-understanding service for the media types an operator names, FIRST in the registry (so it wins for those and the local floor is untouched), ⚡our own one-endpoint contract that a shim puts in front of docling, ⚡URL + profile ride the fingerprint and the bearer never does, ⚡a failure is a RUN failure rather than a quiet fallthrough to prose soup — **shipped (#TBD)** | `SOURCE_PROGRESSIVE`, `EVIDENCE_PARSER_REMOTE` | cost |
| **W7 Linked lane** | `Connector.search()` + `mode: 'linked'`: a source that keeps its own index is ASKED at query time, never walked (a sync of it is refused by name) and its hits ride in the search response's own `linked` section — ⚡beside the ranking, never inside it, because another system's opinion is not this brain's memory and mixing them would make a citation mean two things. ⚡Every call writes a content-free `tool_observation` (0111) and the hits carry its ref, so `ingest_document` closes the loop; ⚡a hit whose observation could not be written is NOT served. Bounded (25 connections, `SOURCE_LINKED_PER_QUERY` each, 8s, every failure named). GitLab is the first implementer (project search, one call per scope, interleaved) — **shipped (#TBD)** | `SOURCE_LINKED` (+ `TOOL_OBSERVATIONS_ENABLED`) | linked mode |
