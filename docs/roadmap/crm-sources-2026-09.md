...
| **W4.2b more vendors + OpenAPI assistant** | `bitrix24`, `kommo`, `hubspot` connectors on the contract — **shipped (#624)**: HubSpot as a connected account (`hubspot` provider, scopes per object, identity by token) or a private-app token, Search-API windows narrowed at the 10 000 cap, associations per page; Bitrix24 on an inbound webhook URL (stored encrypted, code never echoed; OAuth per portal deferred); Kommo on a long-lived token + `baseUrl` (OAuth on the account host deferred to W4.3's per-origin provider lane); the OpenAPI/JSON-sample-driven `RecordsSource` for the long tail with the assistant + live preview — **shipped (#626)**: `rest_records` (config: endpoints per entity, five paging styles, one since-parameter, dotted field paths, relations, deleted flag; auth bearer / basic / header / query) + `POST …/assist` (OpenAPI digest with `$ref` / `allOf` / POST body params, sample answers, conventional-name heuristics always, one strict-schema model call under the flag) + the "Describe the API" step in the connect form | per kind, `SOURCE_MAPPING_ASSISTANT` | one real sandbox per connector (fake vendors in `test/fixtures/fake-crm.ts`; real sandboxes await the operator's apps) |
| **W4.2c Freshness + Salesforce** | inbound webhooks per preset (signature verified, fetch-one); Salesforce preset (SOQL + Bulk 2.0) — **shipped (#628)**: `POST …/:id/webhook` hands out an HMAC-signed address + a secret; the public `POST /v1/source-connections/webhook/:address` is trusted per vendor (HubSpot v3 signature, Pipedrive basic auth, Bitrix24 application token, Kommo / custom REST `?token=` or an HMAC of the body), parsed into entity + id (+ deleted) and applied as a queued `source_sync` job through `fetchRecord` and the records door (a 404 or a deletion closes the facts); `salesforce` on the contract — SOQL over REST with `nextRecordsUrl`, the deleted-ids feed on incremental runs, Bulk API 2.0 for the first walk (`config.bulk`), a connected account (the org from the grant's `instance_url`) or a JWT bearer for an integration user; Salesforce CDC waits for demand | `SOURCE_WEBHOOKS`, `SOURCE_KIND_SALESFORCE` | change at source → fact queryable ≤ 60 s with a webhook (the fetch is one `get` + one door write; the queue's tick is the bound) |
| **W4.3 MCP client OAuth** | RFC 9728 → DCR → PKCE; provider kind `mcp:<origin>`; "Sign in" on MCP sources; W7 linked lane over the official CRM servers' `search` — **shipped (#634)**: `POST …/oauth/mcp/start` discovers the server's authorization server from its 401 (RFC 9728, path-aware well-known fallbacks) and the RFC 8414 / OpenID metadata, registers a public PKCE client (RFC 7591; a secret only when the server issues one) or takes the operator's, consents with PKCE + the RFC 8707 `resource`; the grant is `provider: mcp` + `resource`, kept per resource in `source_oauth_client`, refreshed / revoked through the discovered endpoints; a pack's `auth: 'oauth'` MCP source connects as a grant of the same origin and the harvester runs as it; "Sign in at <origin>" in the connect form. W7's linked lane over the CRM servers' `search` is still W7 | `SOURCE_MCP_OAUTH` | a hosted MCP server signed in from the UI (the fake models the spec; HubSpot's / Pipedrive's servers await the operator's session) |
| **W4.3b Bitrix24 / Kommo as connected accounts** | the two vendors whose OAuth is not the RFC shape — **shipped (#638)**: provider specs with `{host}` templates (Kommo's token endpoint and API on the account's host the callback names in `referer`, accepted under the vendor's suffixes only; Bitrix24's GET token endpoint at `oauth.bitrix.info` answering the portal as `client_endpoint`), a JSON / query token request, `redirect_uri` on refresh, rotated refresh tokens; the connectors take `oauth:<grant>` beside the webhook URL / long-lived token; the connect form offers the account and the pasted alternative | `SOURCE_OAUTH_BITRIX24_CLIENT_ID`, `SOURCE_OAUTH_KOMMO_CLIENT_ID` | a Bitrix24 portal and a Kommo account connected from the UI (the fakes model both flows; real apps await the operator's) |
| **W4.4 `db` on the agent** | read-only Postgres / MySQL / SQLite views → envelopes, DSN never leaves the machine — **shipped (#639)**: the agent's `db` connector (tables / views as record types, keyset pages, a change column for incremental walks else row hashes, foreign keys as relations with the target's name read from its table, read-only sessions, identifiers validated and quoted, `pg` / `mysql2` dynamic, SQLite via `node:sqlite`), `brain-agent db add <name> <dsn>` / `BRAIN_AGENT_DB_<NAME>`, database names in the check-in; the brain: `crm_memory/db` (agent-only), the config contract (a DSN in it is refused), the connection's mapping applied to agent-posted envelopes; the connect form's tables editor and mapping table without a preview | agent config | a self-hosted CRM's DB read where it lives — SQLite e2e; Postgres / MySQL drivers await a sandbox |
